...
void DisplayChunk::CustomHeightMap(std::string m_customHeightMappath)
{
	m_heightmap_path = m_customHeightMappath;
...
void DisplayChunk::SaveHeightMap(ChunkObject * SceneChunk)
{
/*	for (size_t i = 0; i < TERRAINRESOLUTION*TERRAINRESOLUTION; i++)
	...
	// Open The File In Read / Binary Mode.
	pFile = fopen(SceneChunk->heightmap_path.c_str(), "wb+");;
	// Check To See If We Found The File And Could Open It
	if (pFile == NULL)
	{
		// Display Error Message And Stop The Function
