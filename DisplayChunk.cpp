...
void DisplayChunk::UpdatedHeightMap(bool isUpdated)
{
...
void DisplayChunk::PopulateChunkData(ChunkObject * SceneChunk)
{
	...
	{
		m_updatedHeighMap_path = SceneChunk->heightmap_path;
...
void DisplayChunk::LoadHeightMap(std::shared_ptr<DX::DeviceResources>  DevResources)
{
	...
	if (m_isUpdated)
	{
		pFile = fopen(m_updatedHeighMap_path.c_str(), "rb");
	}
	else pFile = fopen(m_heightmap_path.c_str(), "rb");
...
void DisplayChunk::SaveHeightMap()
{
	...
	MessageBox(NULL, L"Terrain has been saved successfully", L"Saved", MB_OK);
	//try not break saving terrain texture
}
