...
void Game::Initialize(HWND window, int width, int height)
{
	...
	//GetClientRect(window, &m_dxClientRect);
#ifdef DXTK_AUDIO
...
void Game::Update(DX::StepTimer const& timer)
{
	...
	if (m_InputCommands.focusCamera|| m_InputCommands.orbCam)
	{
		...
		{
			m_view = m_camera.OrbCamera(m_obj.m_position);
		}
	}
...
void Game::Render()
{
	...
	for (int i = 0; i < numRenderObjects; i++)
	{
		...
		else m_displayList[i].m_model->Draw(context, *m_states, local, m_view, m_projection, false);	//last variable in draw,  make TRUE for wireframe
...
{
	m_displayChunk.SaveHeightMap(SceneChunk);			//save heightmap to file.
}
...
int Game::MousePicking(RECT directXRect, POINT cursorPos)
{
	selectedID = -1;
	...
	const XMVECTOR nearSource = XMVectorSet(cursorPos.x, cursorPos.y, 0.0f, 1.0f);
	const XMVECTOR farSource = XMVectorSet(cursorPos.x, cursorPos.y, 1.0f, 1.0f);
	...
	for (int i = 0; i < m_displayList.size(); i++)
	{
		...
		XMVECTOR nearPoint = XMVector3Unproject(nearSource, 0.0f, 0.0f, directXRect.right, directXRect.bottom, m_deviceResources->GetScreenViewport().MinDepth, m_deviceResources->GetScreenViewport().MaxDepth, m_projection, m_view, local);
		XMVECTOR farPoint = XMVector3Unproject(farSource, 0.0f, 0.0f, directXRect.right, directXRect.bottom, m_deviceResources->GetScreenViewport().MinDepth, m_deviceResources->GetScreenViewport().MaxDepth, m_projection, m_view, local);
...
void Game::updatePickedObject(SceneObject obj)
{
	m_Sobj = obj;
}
void Game::WireFrameToggle()
{
	if (WireFrame)
	{
		WireFrame = false;
